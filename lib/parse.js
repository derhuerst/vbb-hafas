...
const line = (p) => {
	...
	if (p.prodCtx && p.prodCtx.matchId) {
		debug('p.prodCtx.matchId', p.prodCtx.matchId)
		debug('p.prodCtx.num', p.prodCtx.num)
		debug('p.number', p.number)
		debug('p.nameS', p.nameS)
	}
	if (p.prodCtx && p.prodCtx.admin) {
		debug('p.prodCtx.admin', p.prodCtx.admin)
		debug('p.nameS', p.nameS)
	}
	if (r.productCode) {
...
const location = (l) => {
	...
	if (r.id) {
		...
		if (r.id.length < 9) debug('<9 digits ID', l && l.lid, r.id)
